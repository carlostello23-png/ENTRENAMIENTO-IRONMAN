# Plan — IRONMAN 70.3 Cartagena, 29 nov 2026

*(escrito el 17 ago 2026 · 104 días · 15 semanas exactas)*

> `entrenamiento/restricciones.md` **manda sobre este archivo**. Si algo de aquí
> choca con lo que la realidad permite, gana la restricción.

---

## 1. La aritmética del 6:30 — sobre datos reales

*(Reescrito el 17 ago 2026, cuando el atleta aportó los parciales de su 70.3
anterior. **La versión anterior de esta sección está SUPERSEDED** — estaba
construida sobre supuestos y uno de ellos era falso. Ver §1b.)*

### La línea de partida real

| Segmento | Tiempo anterior | Ritmo real |
|---|---|---|
| Natación 1,9 km | **49:00** | 2:34 /100 m |
| Bici 90 km | **3:30:00** | 25,7 km/h |
| Carrera 21,1 km | **2:50:00** | 8:04 /km |
| Suma de segmentos | **7:09** | |
| Con transiciones *(est. ~10 min)* | **~7:19** | |

**Para 6:30 hay que recortar 49 minutos.** Un 11 % del tiempo total.

### De dónde salen esos 49 minutos

| Segmento | Anterior | Objetivo | Recorte | Coste / riesgo |
|---|---|---|---|---|
| **Bici** | 3:30 (25,7 km/h) | **3:10** (28,4 km/h) | **−20 min** | Bajo. Es donde hay más tiempo y ningún riesgo estructural. Acoples + entrenamiento |
| **Carrera** | 2:50 (8:04 /km) | **2:34** (7:18 /km) | **−16 min** | **Alto.** Disciplina más floja, calor de Cartagena, riesgo de lesión a 87 kg |
| **Natación** | 49:00 (2:34 /100 m) | **38:00** (2:00 /100 m) | **−11 min** | **Bajo, y es la sorpresa de este análisis.** Ver §1b |
| Transiciones | ~10 min | 8 min | −2 min | Gratis. Practicarlas |
| **Total** | **~7:19** | **6:30** | **−49 min** | |

### El veredicto, ahora con datos

**El 6:30 es más creíble de lo que parecía, y la razón importa:** ese 7:19 **no
es tu techo, es tu resultado sin entrenar.** Tú mismo lo dijiste — "no entrené
mucho". Recortar un 11 % sobre un resultado sin preparación no es lo mismo que
recortar un 11 % sobre un rendimiento bien entrenado. Lo primero es corriente;
lo segundo sería casi imposible.

Y hay cuatro palancas que se suman, no una:
1. **Entrenar de verdad 15 semanas** (la que faltó la vez anterior)
2. **Bajar 4–6 kg** — vale minutos directos en la carrera a pie
3. **Acoples en la bici** — minutos por poco dinero en 90 km llanos
4. **Aclimatación al calor** — la que casi nadie hace

**Banda realista revisada: 6:35 – 7:00**, con el **6:30 genuinamente en juego**
si el bloque se cumple entero. *(La estimación anterior era 6:30–7:15; se
revisa a mejor porque ahora se sabe que la línea de partida era un resultado sin
entrenar, no un límite.)*

**Sigue habiendo un crux, y es el mismo:** correr a **7:18 /km** al final, con
30 °C y 80 % de humedad. Es 46 s/km más rápido que la vez anterior. Es la parte
más dura del plan y la de más riesgo.

**El punto de control no cambia:**

> **Test de campo, semana 3 (31 ago – 6 sep).** Si el test da ~6:00 /km o mejor,
> el 6:30 se mantiene como objetivo. Si sale claramente peor, se recalibra en la
> revisión de esa semana, con el número delante y sin disimular.

**Objetivo primario:** bajar de 7:00 y correr el medio maratón entero **sin
caminar**.
**Objetivo secundario:** 6:30, condicionado al test de la semana 3.

## 1b. Corrección: la natación pasa de "mantener" a **prioridad**

**Lo que escribí antes estaba mal y se corrige aquí.** Decía que la natación
"rinde poco al reloj" y que había que mantenerla, no maximizarla. Ese
razonamiento se apoyaba en un supuesto **falso**: que nadaba en torno a 35 min.

**Nada en 49 minutos, o sea 2:34/100 m.** Eso no es un parcial rápido de 70.3 en
términos absolutos. Su afirmación de que la natación es su disciplina más fuerte
sigue siendo válida — **es fuerte en relativo a su bici y su carrera**, y no se
discute porque él se conoce. Pero en absoluto **hay ~11 minutos sobre la mesa en
el agua**, y eso lo cambia todo.

**Por qué la natación pasa a ser una de las mejores inversiones del plan:**

- **11 minutos es más que lo que se recorta corriendo con muchísimo más
  sufrimiento y riesgo.**
- **Riesgo de lesión: cero.** Y eso, en un atleta de 87 kg volviendo de cero
  **sin AINEs disponibles**, no es un detalle menor: es forma aerobia gratis sin
  carga estructural.
- **Es su disciplina más fuerte**, o sea la que responde antes al
  entrenamiento — el retorno por hora es alto.
- Nadar más también **protege el plan**: si aparece una molestia corriendo, la
  natación es donde se trasvasa el volumen.

**Consecuencia práctica:** la natación deja de ser "mantenimiento". Sube en la
lista de prioridades de `planificar-semana`, y la skill `analisis-natacion` deja
de decir "no maximizar".

## 2. Principio rector de este bloque

El intento anterior no falló por táctica ni por material. Falló por
**consistencia**. Por lo tanto:

> **La métrica que manda es sesiones completadas / sesiones planificadas.**
> Un plan aburrido que se cumple gana a un plan brillante que se abandona.

Todo lo demás en este archivo está subordinado a eso.

## 3. Las cuatro fases

Trabajado **hacia atrás desde el 29 de noviembre**.

### Fase 0 — Reactivación · Semanas 1–3 · 17 ago – 6 sep · 4–5,5 h/sem

**Trabajo de la fase:** volver a moverse todos los días y preparar tendones y
articulaciones. **No es una fase de carga.**

- **Cero intensidad.** Todo en conversación completa.
- Frecuencia por encima de duración: mejor 5 sesiones de 40 min que 2 de 100.
- La carrera arranca **muy** conservadora — es donde se rompe la gente que
  vuelve de cero.
- Empieza el registro de FC en reposo y HRV con el Garmin: **hacen falta ~14
  días para tener una línea base**. Sin línea base no hay chequeo de
  recuperación que valga.
- **Termina con el test de campo** (semana 3).

**Razonamiento:** desde cero, el sistema cardiovascular se adapta en semanas,
pero tendones, fascia y hueso tardan meses. La lesión típica de este bloque
llega en la semana 4–6 por haber corrido demasiado en la 1–2.

### Fase 1 — Base · Semanas 4–8 · 7 sep – 11 oct · 6–8,5 h/sem

**Trabajo de la fase:** construir el motor aeróbico y la resistencia estructural
para aguantar las distancias.

- El grueso sigue siendo **aeróbico suave**. La mayoría del volumen, fácil.
- **Se desbloquea el trabajo de calidad** — pero solo si el test de la semana 3
  se hizo de verdad. Sin test, se sigue por RPE.
- Crece la salida larga de bici de fin de semana y la tirada larga.
- Entran los primeros **ladrillos** (bici → carrera) cortos.
- **Semana 7 es de descarga.** No es opcional.

### Fase 2 — Específico · Semanas 9–13 · 12 oct – 15 nov · 7–11 h/sem

**Trabajo de la fase:** convertir la forma general en forma de carrera, y
preparar el cuerpo para Cartagena en concreto.

- Ladrillos largos con la carrera a **ritmo objetivo**, no a tope.
- La salida larga de bici se acerca a la distancia y al esfuerzo de carrera.
- **Arranca la aclimatación al calor** (ver protocolo). Esta es la parte que casi
  nadie hace y que más minutos vale en Cartagena.
- Se ensaya **nutrición e hidratación de carrera** en cada sesión larga — nunca
  se estrena nada el día de la carrera.
- **Semana 11 es de descarga.**
- **Semana 13 es el pico.** Después, solo se baja.

### Fase 3 — Afinamiento · Semanas 14–15 · 16 – 29 nov · taper

**Trabajo de la fase:** llegar descansado sin perder chispa.

- El **volumen baja mucho; la intensidad se mantiene** en dosis pequeñas. Bajar
  las dos cosas deja las piernas planas.
- **La aclimatación al calor se mantiene hasta el final** — se pierde en ~1–2
  semanas si se abandona.
- Se cierra el plan de carrera: parciales, nutrición, transiciones, plan B.
- Viaje a Cartagena: llegar con margen para acusar el cambio de altitud y calor.

## 4. Volumen semanal objetivo

| Sem | Fechas | Fase | Horas | Nota |
|---|---|---|---|---|
| 1 | 17–23 ago | Reactivación | 4,0 | Arranque desde cero |
| 2 | 24–30 ago | Reactivación | 5,0 | |
| 3 | 31 ago–6 sep | Reactivación | 5,5 | **TEST DE CAMPO** |
| 4 | 7–13 sep | Base | 6,5 | Se desbloquea calidad |
| 5 | 14–20 sep | Base | 7,5 | |
| 6 | 21–27 sep | Base | 8,0 | |
| 7 | 28 sep–4 oct | Base | 6,0 | **Descarga** |
| 8 | 5–11 oct | Base | 8,5 | |
| 9 | 12–18 oct | Específico | 9,5 | Empieza calor |
| 10 | 19–25 oct | Específico | 10,5 | |
| 11 | 26 oct–1 nov | Específico | 7,0 | **Descarga** |
| 12 | 2–8 nov | Específico | 10,5 | |
| 13 | 9–15 nov | Específico | 11,0 | **Pico** |
| 14 | 16–22 nov | Afinamiento | 7,0 | Taper |
| 15 | 23–29 nov | Afinamiento | 4,0 | **CARRERA domingo 29** |

Techo de 11 h dictado por la restricción real: 5 h entre semana (1 h/día) +
fin de semana.

### El déficit calórico va acoplado a esta tabla

Bajar grasa es objetivo declarado, y **empuja el 6:30** en vez de competir con
él: menos masa mejora la economía de carrera, disipa mejor el calor y reduce la
carga de impacto. El conflicto no es de objetivos, es de **ritmos**.

La solución es periodizarlo al revés del volumen — **déficit cuando el volumen
es bajo, mantenimiento cuando es alto**:

| Fase | Semanas | Déficit |
|---|---|---|
| Reactivación | 1–3 | **Moderado.** La mejor ventana del bloque |
| Base | 4–8 | **Ligero** |
| Específico | 9–13 | **Mantenimiento.** Pico de carga + calor: no se recorta energía |
| Taper | 14–15 | **Mantenimiento o ligero superávit** |

**Prohibido el déficit** en las semanas de test (3, 8, 12), en el taper y en la
semana de carrera. Detalle completo y señales de alarma en
`salud/protocolos/deficit-responsable.md`.

## 5. Progresión — por disciplina, nunca sobre el total

La regla del ~10 % se aplica **a cada disciplina por separado**. Partiendo de
cero, cualquier disciplina que reciba carga muestra un salto porcentual enorme
contra una historia vacía: eso es aritmética, no fisiología.

**La carrera a pie lleva el límite más estricto**, porque es la disciplina más
floja, la de mayor impacto estructural y la que decide el resultado:

| | Regla |
|---|---|
| **Carrera** | Máx. **+10 % de tiempo semanal**. La tirada larga sube máx. **10 min por semana** y nunca dos semanas seguidas de subida grande. Techo: ~1 h 45. |
| **Bici** | Máx. **+15 % semanal.** Más tolerante: sin impacto. |
| **Natación** | Flexible. Punto fuerte, bajo riesgo, poco retorno al reloj. Mantener, no maximizar. |
| **Fuerza** | 2 sesiones/semana constantes. No progresa en volumen, sí en carga. |

**Ante cualquier molestia en la carrera a pie: se corta el volumen de carrera,
no el de bici ni el de natación.** Perder 2 semanas corriendo por una
tendinopatía cuesta más que todo lo que se gane apretando ahora.

## 6. La brecha — dónde estamos contra la distancia de carrera

Este es el panel que más importa y el que el dashboard muestra arriba.

| | Sesión más larga hoy | Distancia de carrera | Brecha | Ritmo objetivo |
|---|---|---|---|---|
| Natación | 0 | 1,9 km | Completa | ~2:00 /100 m *(antes 2:34)* |
| Bici | 0 | 90 km | Completa | ~28,4 km/h *(antes 25,7)* |
| Carrera | 0 | 21,1 km | Completa | ~7:18 /km *(antes 8:04)* |

Base cero: las tres brechas están abiertas al 100 %. Se actualiza cada semana
con datos reales de Garmin. La columna de ritmo objetivo sale de §1 y da una
referencia concreta contra la que medir cada sesión.

## 7. Lo que decide este plan y aún no está resuelto

1. **Capacidad real corriendo — DESCONOCIDA.** Es la variable que determina si
   6:30 es alcanzable. Se resuelve en el test de la semana 3.
2. **Aclimatación al calor.** Entrenar a 14 °C y competir a 30 °C con humedad
   alta. Sin protocolo, el ritmo de carrera se cae aunque la forma sea buena.
3. **Sin neopreno.** El agua de Cartagena está muy por encima del límite para
   usarlo. Nadar sin neopreno es más lento y exige más del tren superior. Hay
   que entrenar así.
4. **Nutrición en carrera — sin definir.** ~4 h de esfuerzo con calor extremo.
   El plan de hidratación y sales se ensaya desde la fase 2, nunca se estrena el
   día de la carrera.
5. 🔴 **La inscripción no está hecha** *(al 17 ago 2026)*. Es lo más urgente de
   todo el proyecto: hoy se está entrenando para una carrera sin plaza.
6. **Tiempo y parciales del 70.3 que ya completó — DESCONOCIDOS.** Es el dato
   que más ayudaría a calibrar el 6:30, por encima del test de campo. Un tiempo
   real de 70.3, aunque saliera mal, es una referencia que ningún test da.
7. **Historial de lesiones, otras condiciones crónicas y medicación —
   DESCONOCIDOS.**

### Resuelto el 17 ago 2026

- ✅ **Aguas abiertas: sí, varias veces**, incluida una salida de 70.3. Deja de
  ser brecha. No hay que gastar semanas en sighting ni salidas en masa.
- ✅ **Bici: Trek Émonda SL, carbono, Shimano 105.** El parcial de 3:10 pasa a
  ser cuestión de motor, no de material — con esa bici, 28,4 km/h en 90 km
  llanos es perfectamente alcanzable.
- ⚠️ **No tiene acoples, y se plantea comprarlos. Recomendado.** La Émonda es
  bici de escalador; en 90 km llanos la **posición vale más minutos que el peso
  de la bici**, y unos acoples de clip cuestan una fracción de cualquier otra
  mejora. **Condición:** se instalan **antes de la semana 9** y se entrena con
  ellos — cambian la posición, el manejo y la exigencia sobre lumbares y cuello.
  Nunca se estrena una posición nueva el día de la carrera.
- ✅ **Sin historial de lesiones.** Partimos sin deuda estructural. **No es
  permiso para progresar más rápido.**
- 🎯 **Bajar grasa es objetivo declarado.** Se integra como déficit periodizado
  (ver §4 arriba). Va a favor del 6:30, no en contra.
- ⚠️ **Alergia a los AINEs** — elevada a regla de seguridad en `CLAUDE.md`.
  **Endurece el límite de progresión de la carrera a pie**: sin la opción de
  medicarse, una molestia mal gestionada va directa a semana perdida, y aquí no
  hay semanas de sobra.
- ⚠️ **87 kg, 35 años.** Dos efectos reales, ambos sobre la carrera a pie: la
  fuerza de impacto escala con la masa, y más masa produce más calor con peor
  disipación — justo lo que castiga Cartagena. **Sube la prioridad del protocolo
  de calor y del límite del 10 %.** *(Sin la talla no se interpreta como
  composición corporal ni se prescribe pérdida de peso.)*

---

## Registro

- **2026-08-17** — Plan creado. 15 semanas, 4 fases, objetivo 6:30 condicionado
  al test de la semana 3.
